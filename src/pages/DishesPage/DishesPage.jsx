...
const LazyRecipeContent = React.lazy(() =>
    import("../../features/RecipeContent/RecipeContent")
);
...
export const DishesPage = () => {
    ...
    const findSuggestions = useMemo(() => {
        return (item) => {
            ...
            return item.suggest
                .map((elem) =>
                    mock_menus_standards.find(
                        (element) => element.productId === elem
                    )
                )
                .filter(Boolean);
        };
    ...
    useEffect(() => {
        ...
        const fetchProductById = () => {
            try {
                const foundProduct = mock_menus_standards.find(
                ...
                const currentIndex = mock_menus_standards.findIndex(
                    (item) => item.productId === parseInt(productId)
                );

                // Вычисляем все данные за один проход
                const nextProd =
                    currentIndex < mock_menus_standards.length - 1
                ...
                if (isMounted) {
                    setProducts({
                        ...
                        nextSuggest: nextProd
                            ? findSuggestions(nextProd)
                            : null,
                        prevSuggest: prevProd
                            ? findSuggestions(prevProd)
                            : null,
                    });
                ...
                if (nextProd) {
                    const nextNextProd =
                        currentIndex + 1 < mock_menus_standards.length - 1
    ...
    if (!products.current)
        return <div className="not-found">Блюдо не найдено</div>;
